...
unlock({button,Digital}, {SoFar,Code}) ->
	io:format("I:[unlock] ~p SoFar:~p  Code:~p~n",[Digital,SoFar,Code]),
...
open({button,Data}, State) ->
	io:format("I:[open] Data:~p  State:~p~n",[Data,State]),
	do_lock(),
	io:format("~p~n",[Data]),
	timer:sleep(1000),
...
unlock(open, _From, State) ->
	io:format("I:[unlock/3] State:~p~n",[State]),
	do_unlock(),
