...
one_state(nextEvent, {TimeStyle,_}) ->
	Time_list=time2Lists(os:timestamp()),
	io:format("T:~p~n",[Time_list]),
	Time=lookupNextTime(TimeStyle),
	Ref=gen_fsm:send_event_after(Time, nextEvent),
...
start_link(TimeStyle) ->
	 case is_timeStyle(TimeStyle) of
		 true->gen_fsm:start_link({local,?MODULE}, ?MODULE, [TimeStyle], []);
		 false->{error,"badarg"}
	 end.
...
%% 距离下一次记录的ms时间
lookup_time()->
	gen_fsm:send_all_state_event(?MODULE, lookup_time).
